...
«Ἀναστάσεως ἡμέρα λαμπρυνθῶμεν Λαοί, Πάσχα Κυρίου, Πάσχα· ἐκ γὰρ θανάτου
πρὸς ζωήν, καὶ ἐκ γῆς πρὸς οὐρανόν, Χριστὸς ὁ Θεός, ἡμᾶς διεβίβασεν,
ἐπινίκιον ᾄδοντας».
...
\hypertarget{sec-about_lemmatization}{%
\chapter{Лемматизация}\label{sec-about_lemmatization}}

Лемматизация является одной из базовых NLP-задач -- приведение всех слов
в тексте к начальным формам.

При решении задачи параллельного выравнивания текстов лемматизация может
понадобится для дальнейшей обработки текста с помощью элайнеров или
пайплайна, использующего словарь переводных эквивалентов. Мы столкнулись
с этой необходимостью при экспериментах с \emph{LF-aligner}.

В других ситуациях -- например, при использовании векторных моделей
наподобие \emph{Lingtrain} -- необходимость в лемматизации текстов
пропадает.

\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, rightrule=.15mm, breakable, arc=.35mm, left=2mm, colback=white, bottomrule=.15mm, toprule=.15mm, leftrule=.75mm, opacityback=0]

\textbf{Лемматизация на этапе постпроцессинга}\vspace{2mm}

При дальнейшей работе над проектом улучшение качества лемматизации
текстов следует выделить в отдельную исследовательскую задачу,
актуальную для двух направлений развития проекта:
...
\item
  ...
  \protect\hyperlink{sec-about_evluation}{качество выравнивания}
\end{itemize}

\end{tcolorbox}

\hypertarget{sec-lemmatizing_pent}{%
\section{Лемматизация текстов Цветной
триоди}\label{sec-lemmatizing_pent}}

\hypertarget{sec-lemm_csl}{%
\subsection{Церковнославянский текст}\label{sec-lemm_csl}}
...
\emph{Taiga} справлялась лучше, чем SynTagRus и GSD. Например, ей
удалось распарсить словоформу \emph{\texttt{вечерни\ (вечерня)}}, с
которой из других моделей справилась только \emph{Torot}.
...
экспериментов мы решили отказаться от \emph{PROIEL}, а из современных
моделей выбирать между \emph{Taiga} и \emph{SynTagRus}, хотя их качество
при лемматизации службы одного дня оказалось не совсем идеальным
...
\textbf{Результат лемматизации CLTK}\vspace{2mm}

\begin{quote}
ὁ κύριος ἀναλαμβάνω εἰς οὐρανός ἵνα πέμπω ὁ παράκλητος ὁ κόσμῳ, ὁ
...
\textbf{Результат лемматизации UDPipe - Perseus}\vspace{2mm}

\begin{quote}
ὁ Κύριος ἀαλαμβάνω εἰς οὐρανός, ἵνα πέμπω ὁ Παράκλητος ὁ κόσμος , ὁ
οὐρανός ἑτοιμάζω ὁ θρόνος αὐτός , νεφέλα ὁ ἐπίβασις αὐτός , Ἄγγελοι
...
точность. \emph{CLTK} не справился со словом \emph{\texttt{ὁ\ κόσμος}}
(здесь в значении «мир»), оставив его в форме дательного падежа.
Возможной причиной ошибки может являться \emph{iota subscriptum}
(подстрочная йота), встречающаяся, среди прочего, в формах
существительных дательного падежа.

\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, rightrule=.15mm, breakable, arc=.35mm, left=2mm, colback=white, bottomrule=.15mm, toprule=.15mm, leftrule=.75mm, opacityback=0]

\textbf{Что можно с этим сделать?}\vspace{2mm}

Потенциальным решением подобной проблемы может быть замена таких букв на
формы \emph{iota adscriptum} на этапе предварительной обработки текста:
\texttt{ῳ\ –\textgreater{}\ ωι}
...
Не смотря на то, что изначальные исследовательские цели проекта не
финализированы на данном этапе, работа над проектом проявила множество
особенностей обрабатываемых текстов.

Мы прояснили \protect\hyperlink{sec-about_pent_specifics}{специфику
текстов} Цветной Триоди, важность
\protect\hyperlink{sec-html_color}{цветовой разметки} для определения
внутренней структуры текстов, оценили эффективность работы готовых
\protect\hyperlink{sec-about_aligners}{инструментов-элайнеров}, провели
серию экспериментов по
\protect\hyperlink{sec-about_lemmatization}{лемматизации} билингвального
корпуса, а также отметили наиболее проблемные аспекты и предложили
\protect\hyperlink{sec-about_txt_similarity}{идеи} для поиска решений.

Проект продолжается.

Мы -- Настя и Лев -- продолжаем исследовать изложенные в этом описании
методы и инструменты, а также искать новые решения, которые могут
приблизить проект к искомому результату.

\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, rightrule=.15mm, breakable, arc=.35mm, left=2mm, colback=white, bottomrule=.15mm, toprule=.15mm, leftrule=.75mm, opacityback=0]

\textbf{Что дальше?}\vspace{2mm}
